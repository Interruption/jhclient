...
public class MapView extends Widget implements DTarget, Console.Directory {
	...
	static class WrapCam extends Camera {
		public final Coord region = new Coord(10, 10);
	...
	static class CakeCam extends Camera {
		private Coord border = new Coord(10, 10);
	...
	private void drawtile(GOut g, Coord tc, Coord sc) {
		...
		//        g.text(tc.toString(), sc);
		//	Utils.drawtext(g, t.toString(), sc);
	...
	private void draw_tile_grid(GOut g, Coord tc, Coord sc) {
		...
		g.chcolor(new Color(0.2f,0.2f,0.2f,1.0f));
		g.line(c2, c1, 1);
		g.line(c1.add(1, 0), c4.add(1, 0), 1);
...
//								objtext[0] = "";
	...
	public void map_move_step(int x, int y) {
		...
		wdgmsg("click", ark_bot.GetCenterScreenCoord(), mc, btn, modflags );    	    	    	
	}
		...
		synchronized(glob.oc) {
			...
			for(KinInfo k : kin) {
				...
				if(gc.isect(Coord.z, sz)) {
					...
					if((k.gob == gob_at_mouse) || (Config.always_show_nicks)) {
						show = Color.WHITE;
	...
	private void checkplmove() {
		...
		long now = System.currentTimeMillis();
		// (playergob >= 0) &&
		if( ((pl = glob.oc.getgob(playergob)) != null) && (pl.sc != null)) {
			Coord plp = pl.getc();
	...
	public void update(long dt) {
		...
		if ((Config.auto_start_script.length() > 0) && time_to_start <= 0) {
			...
			try {
				started = true;
				time_to_start = 0; 
				ark_bot.StartScript( Config.auto_start_script );
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
